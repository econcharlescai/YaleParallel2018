...
int main(int argc, const char * argv[]) {
    ...
    // We create Policy matrix, which will
    // store the optimal policies corresponding to each (nk, ntheta)
    // combination
    
    MatrixXd Policy(nk, ntheta);
    
    // Matrix for storing and printing data in desired format
    ...
    /*
     ...
     In either case, we initialize ValNew by seting it equal to ValOld.
     */
    ...
    // Extract Value and Policy functions from the corresponding text files.
        ...
        for (int j=0; j<ntheta; j++) {
            ...
            /* 
            Dump the value of the first column (corresponding to the kgrid) and extract the values of the value and policy functions
            in the text file.
            */
    ...
    /* 
    Declare the Value Function Iteration Function. This function takes the old value function guess as an argument and 
    returns a 2nk x ntheta matrix consisting of a concatenation of the new value function guess and the policy matrix. 
    More specifically, the first nk x ntheta block of this matrix is the new value function guess whereas the last block is
    the Policy function.
    */
    ...
    time = -omp_get_wtime();    // Start the timing of the iterations
    for (int i=numstart; i<Numits; i++) {
        
        /* 
        ValNew computed by the previous Value function iteration now becomes the ValOld of the current iteration.
        */

        ValOld=ValNew;
        
        // Invoke Valit, and store the Concatenated matrix Valit returns in the matrix Result
        Result=ValIt(ValOld);
        
        // Extract Value Function and Policy Function and compute the maximum difference between the old and new value function
        ValNew=Result.topLeftCorner(nk,ntheta);
        Policy=Result.bottomRightCorner(nk, ntheta);
     // KEN DOES NOT UNDERSTAND THIS LINE
        ...
        if (i % datafreq==0) {
            ...
            /* 
            For each theta state we create a textfile with three columns. The first column is the kgrid of the problem, 
            the second column contains the corresponding values of the value function, and the last column contains 
            the corresponding optimal policies
             */
    ...
    time+=omp_get_wtime();
    printf("Time taken for Numits iterations: %.2fs\n", time);
