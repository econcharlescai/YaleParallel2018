 ...
 * 
 * Since we are using MPI to parallelize the value function iteration 
 * over the capital states, the MPI_Comm_World communicator splits 
 * the nk states among the MPI processes within it and collects 
 * the results in the rank 0 process.
 * 
...
MatrixXd ValIt(MatrixXd ValOld=MatrixXd::Zero(nk, ntheta)) {
    ...
    // Obtain number of proccesses and their id's
    ...
    /* Create and initialize ValNew, which will store the new value function guess 
     * after the iteration. We initially set ValNew equal to the Old Value Function Guess. 
    ...
    /* We now proceed to split the number of capital states among the MPI Processes within MPI_Comm_World.
     ...
     an extra capital stock each, where r is the remainder that results from dividing nk by MPI_COMM_World size.
    ...
    /* The first r processes will each send 1+ nk/size elements to each 
     * of the receive buffers, since they each were assigned an extra capital stock. 
    ...
    /*
     ...
     and choosing the one that yields the highest value. These policies and 
     new values will then be part of our new value and policy function guesses.
     
     The following makes use of MPI to parallelize over the capital states.
     */
